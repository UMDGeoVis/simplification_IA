...
public:
    ...
    inline Topo_Sempl(Arc *arc, double val, int lvl, int filt_s0, int filt_s1, vector<int> filt_ex)
    {
        this->arc = arc;
        this->val = val;
        this->lvl = lvl;
        this->filt_s0 = filt_s0;
        this->filt_s1 = filt_s1;
        this->filt_ex = filt_ex;
    }
};
...
public:
    ...
    inline vector<int> sort_simplex(vector<int> *simplex)
    {
        ...
        {
            if (i == 0)
                ordered.push_back(simplex->at(0));
            else
            {
                ...
                if (it == ordered.begin())
                    ordered.push_front(simplex->at(i));
                else if (it == ordered.end())
                    ordered.push_back(simplex->at(i));
                else
                    ordered.insert(it, simplex->at(i));
            }
        }
    ...
    inline void push_ordered(list<vector<int>> *coda, vector<int> &simplesso)
    {
        ...
        {
            if (it == coda->begin())
                coda->push_front(simplesso);
            else if (it == coda->end())
                coda->push_back(simplesso);
            else
                coda->insert(it, simplesso);
        }
    }
    ...
    inline bool same_side_of_edge(int v1, int v2, int e1, int e2)
    {
        Vertex2D va = mesh->getVertex(e1);
        Vertex2D vb = mesh->getVertex(e2);
        Vertex2D v1_obj = mesh->getVertex(v1);
